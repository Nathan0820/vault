...
```mermaid
flowchart LR
    subgraph Proof ["Valid Reduction Proof"]
        P1["1. Transformation runs in poly-time"]
        P2["2. (⇒) If α is YES for A,<br/>then β is YES for B"]
        P3["3. (⇐) If β is YES for B,<br/>then α is YES for A"]
    end
...
| [[L8]] | Dynamic programming for problems like KNAPSACK |
