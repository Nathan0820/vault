...
- **The inode** — the actual data + metadata (permissions, size, timestamps) stored on disk
- **The filename/directory entry** — just a human-readable _name_ that _points to_ an inode

Think of an inode as a **locker** 🗄️, and the filename as a **label stuck on the locker door**.
...
- If you delete `original.txt`, the data is **still safe** — because `hardlink.txt` still points to the same inode
- The inode keeps a **reference count** — data is only truly deleted when that count hits 0
...
- It's like a **road sign** 🪧 pointing you somewhere else
- If you delete `original.txt`, the symlink becomes a **broken/dangling link** 💔

> 🧠 Think of it like a sticky note that says "the file is over there →". If the file moves, the note is useless!
...
| 📚 Lecture | 🔗 Connection |
| :--- | :--- |
| [[content/CS2106/L1]] | OS services — file system is a core OS abstraction for persistent storage |
| [[content/CS2106/L2]] | Process context — file descriptors are part of the OS context |
| [[content/CS2106/L3]] | Scheduling — I/O-bound processes spend time blocked on file operations |
| [[content/CS2106/L4]] | IPC — pipes use file descriptors for communication |
| [[content/CS2106/L6]] | Synchronization — concurrent file access needs locks |
| [[L7]] | Memory vs file management — similar allocation problems, different persistence |
| [[L8]] | Paging concepts — file buffer cache uses similar mechanisms |
| [[L9]] | Page replacement — LRU/CLOCK algorithms inform buffer cache policies |
| [[L11]] | File system implementation — how files are stored on disk |
| [[L12]] | File system case studies — FAT, Ext2 real-world designs |
